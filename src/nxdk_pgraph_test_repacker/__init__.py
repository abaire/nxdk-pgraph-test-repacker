...
from python_xiso_repacker import (  # type: ignore[import-untyped]
    ensure_extract_xiso,
    extract_file,
    replace_file,
)
from python_xiso_repacker.util.github import (  # type: ignore[import-untyped]
    download_artifact,
...
def download_latest_iso(output_path: str | PathLike) -> bool:
    ...
    info = fetch_github_release_info(_NXDK_PGRAPH_TESTS_REPO_API)
    if not info:
        logger.error("Failed to fetch release info for nxdk_pgraph_tests")
        return False
    ...
    for asset in info.get("assets", []):
        ...
        return download_artifact(str(output_path), download_url)

    logger.error("Failed to find ISO in release assets")
    return False
...
def run(argv: Sequence[str] | None = None) -> None:
    ...
    source.add_argument(
        ...
        "-d",
        nargs="?",
        const="nxdk_pgraph_tests_xiso-latest.iso",
        default=None,
    ...
    if args.iso:
        ...
        iso_file = args.download or "nxdk_pgraph_tests_xiso-latest.iso"
        if not download_latest_iso(iso_file):
            sys.exit(1)
